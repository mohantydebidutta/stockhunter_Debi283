...
# ---- FULL NSE universe (all ~2000 listed equities) from NSE's official CSV ----
# Cached in memory + on disk so we only download it once a day.
_NSE_FULL = {"syms": [], "names": {}, "ts": 0}

def _fetch_full_nse():
    """Download every NSE equity symbol + company name from NSE's official list.
    Returns (symbols_list, {name_lower: symbol}). Empty on failure."""
    now = time.time()
    if _NSE_FULL["syms"] and (now - _NSE_FULL["ts"] < 86400):   # 1 day cache
        return _NSE_FULL["syms"], _NSE_FULL["names"]
    # try local cache file first (survives between runs)
    cache_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "nse_full_cache.json") \
        if "__file__" in globals() else "nse_full_cache.json"
    try:
        if os.path.exists(cache_path) and (now - os.path.getmtime(cache_path) < 86400):
            with open(cache_path, "r", encoding="utf-8") as f:
                d = _json.load(f)
            if d.get("syms"):
                _NSE_FULL.update({"syms": d["syms"], "names": d.get("names", {}), "ts": now})
                return _NSE_FULL["syms"], _NSE_FULL["names"]
    except Exception:
        pass
    ...
    try:
        ...
        for row in rdr:
            ...
            nm = row[1].strip()
            series = row[2].strip()
            if series not in ("EQ", "BE"):   # only normal equity series
                continue
            if not sym:
                continue
            syms.append(sym)
            if nm:
                names[nm.lower()] = sym
        if syms:
            _NSE_FULL.update({"syms": syms, "names": names, "ts": now})
            try:
                with open(cache_path, "w", encoding="utf-8") as f:
                    _json.dump({"syms": syms, "names": names}, f)
            except Exception:
                pass
            return syms, names
    except Exception:
...
def full_universe():
    """Every tradeable symbol for the current market.
    For India this is the WHOLE NSE (downloaded live); falls back to NIFTY1000."""
    if MARKET == "US":
        return list(US_STOCKS)
    syms, _ = _fetch_full_nse()
    if syms:
        return syms
    return list(NIFTY1000)   # fallback if NSE download fails
...
# ---- company name -> symbol (so users can search by NAME or symbol) ----
# Maps common company names / keywords to the NSE symbol. Lowercase keys.
INDIA_NAMES = {
    ...
    "ltimindtree":"LTIM","mindtree":"LTIM","persistent":"PERSISTENT","coforge":"COFORGE",
    "hdfc bank":"HDFCBANK","hdfc":"HDFCBANK","hdfcbank":"HDFCBANK",
    "icici bank":"ICICIBANK","icici":"ICICIBANK","axis bank":"AXISBANK","axis":"AXISBANK",
    "kotak":"KOTAKBANK","kotak mahindra":"KOTAKBANK","indusind":"INDUSINDBK",
    "idfc first":"IDFCFIRSTB","sbi":"SBIN","state bank":"SBIN","sbin":"SBIN",
    "bank of baroda":"BANKBARODA","bob":"BANKBARODA","pnb":"PNB","punjab national":"PNB",
    "canara":"CANBK","canara bank":"CANBK","union bank":"UNIONBANK","indian bank":"INDIANB",
    "tata motors":"TATAMOTORS","tatamotors":"TATAMOTORS","mahindra":"M&M","m&m":"M&M",
    "maruti":"MARUTI","maruti suzuki":"MARUTI","bajaj auto":"BAJAJ-AUTO",
    "eicher":"EICHERMOT","royal enfield":"EICHERMOT","hero":"HEROMOTOCO","hero motocorp":"HEROMOTOCO",
    "tvs":"TVSMOTOR","tvs motor":"TVSMOTOR",
    "ongc":"ONGC","ioc":"IOC","indian oil":"IOC","bpcl":"BPCL","gail":"GAIL",
    "ntpc":"NTPC","power grid":"POWERGRID","powergrid":"POWERGRID","coal india":"COALINDIA",
    "hul":"HINDUNILVR","hindustan unilever":"HINDUNILVR","unilever":"HINDUNILVR",
    "itc":"ITC","nestle":"NESTLEIND","britannia":"BRITANNIA","dabur":"DABUR",
    "marico":"MARICO","tata consumer":"TATACONSUM",
    "sun pharma":"SUNPHARMA","sunpharma":"SUNPHARMA","dr reddy":"DRREDDY","drreddy":"DRREDDY",
    "cipla":"CIPLA","divis":"DIVISLAB","divis lab":"DIVISLAB","aurobindo":"AUROPHARMA",
    "lupin":"LUPIN","torrent pharma":"TORNTPHARM",
    "tata steel":"TATASTEEL","tatasteel":"TATASTEEL","jsw steel":"JSWSTEEL","jsw":"JSWSTEEL",
    "hindalco":"HINDALCO","vedanta":"VEDL","vedl":"VEDL","sail":"SAIL","nmdc":"NMDC",
    "jindal":"JINDALSTEL","jindal steel":"JINDALSTEL",
    "ultratech":"ULTRACEMCO","ultracemco":"ULTRACEMCO","shree cement":"SHREECEM",
    "ambuja":"AMBUJACEM","acc":"ACC","dalmia":"DALBHARAT",
    "bajaj finance":"BAJFINANCE","bajfinance":"BAJFINANCE","bajaj finserv":"BAJAJFINSV",
    "cholamandalam":"CHOLAFIN","chola":"CHOLAFIN","shriram":"SHRIRAMFIN","muthoot":"MUTHOOTFIN",
    "airtel":"BHARTIARTL","bharti airtel":"BHARTIARTL","bharti":"BHARTIARTL",
    "vodafone idea":"IDEA","vi":"IDEA","idea":"IDEA",
    "lt":"LT","l&t":"LT","larsen":"LT","larsen toubro":"LT","siemens":"SIEMENS",
    "abb":"ABB","bhel":"BHEL","bel":"BEL","bharat electronics":"BEL","cummins":"CUMMINSIND",
    "adani enterprises":"ADANIENT","adani ent":"ADANIENT","adani ports":"ADANIPORTS",
    "adani power":"ADANIPOWER","adani green":"ADANIGREEN","adani":"ADANIENT",
    "zomato":"ZOMATO","paytm":"PAYTM","nykaa":"NYKAA","dmart":"DMART","avenue supermarts":"DMART",
    "irctc":"IRCTC","dixon":"DIXON","trent":"TRENT","titan":"TITAN","asian paints":"ASIANPAINT",
    "pidilite":"PIDILITRIND","havells":"HAVELLS","biocon":"BIOCON","ltts":"LTTS",
}

US_NAMES = {
    "apple":"AAPL","aapl":"AAPL","microsoft":"MSFT","msft":"MSFT",
    "google":"GOOGL","alphabet":"GOOGL","googl":"GOOGL","amazon":"AMZN","amzn":"AMZN",
    "meta":"META","facebook":"META","nvidia":"NVDA","nvda":"NVDA","tesla":"TSLA","tsla":"TSLA",
    "netflix":"NFLX","nflx":"NFLX","amd":"AMD","intel":"INTC","intc":"INTC",
    "palantir":"PLTR","pltr":"PLTR","coinbase":"COIN","uber":"UBER","lyft":"LYFT",
    "disney":"DIS","walmart":"WMT","jpmorgan":"JPM","jp morgan":"JPM","visa":"V",
    "mastercard":"MA","boeing":"BA","starbucks":"SBUX","mcdonalds":"MCD","nike":"NKE",
    "paypal":"PYPL","salesforce":"CRM","oracle":"ORCL","adobe":"ADBE","qualcomm":"QCOM",
    "broadcom":"AVGO","micron":"MU","ford":"F","gm":"GM","general motors":"GM",
    "berkshire":"BRK-B","exxon":"XOM","chevron":"CVX","pfizer":"PFE","moderna":"MRNA",
}

def resolve_symbol(text):
    """Turn a user's text (company name OR symbol) into a tradeable symbol."""
    if not text: return text
    t=text.strip()
    key=t.lower()
    names = US_NAMES if MARKET=="US" else INDIA_NAMES
    if key in names: return names[key]
    # try a loose contains match on the name keys
    for nm,sym in names.items():
        if key==nm or (len(key)>=4 and key in nm):
            return sym
    return t.upper()

def search_suggestions(query, limit=12):
    """Return [(display_name, symbol)] matching a query, for a search dropdown."""
    q=(query or "").strip().lower()
    names = US_NAMES if MARKET=="US" else INDIA_NAMES
    syms = US_STOCKS if MARKET=="US" else NIFTY1000
    out=[]; seen=set()
    if q:
        # name matches first
        for nm,sym in names.items():
            if q in nm and sym not in seen:
                out.append((f"{nm.title()} ({sym})", sym)); seen.add(sym)
        # then symbol matches
        for s in syms:
            if q in s.lower() and s not in seen:
                out.append((s, s)); seen.add(s)
    if not out:
        out=[(s,s) for s in syms[:limit]]
    return out[:limit]
...
#  ENGINE
# ----------------------------------------------------------------------
# ---- free fundamentals via Yahoo quoteSummary (cookie + crumb) ----
# Yahoo's plain .info often comes back EMPTY for Indian (.NS/.BO) tickers.
# Hitting quoteSummary directly with a cookie+crumb is far more reliable and
# returns INR figures (ROE, Debt/Equity, P/E, P/B, Market Cap) - all free, no key.
...
def _yf_session():
    """Build (and cache for 30 min) a Yahoo opener with cookie + crumb."""
    now = time.time()
    if _YF_SESSION["opener"] is not None and (now - _YF_SESSION["ts"] < 1800) and _YF_SESSION["crumb"]:
    ...
    try:
        ...
        # 1) get cookies
        try:
            opener.open(urllib.request.Request("https://fc.yahoo.com", headers=hdr), timeout=10).read()
        except Exception:
            pass  # cookie endpoint sometimes 404s but still sets the cookie
        # 2) get a crumb tied to those cookies
        req = urllib.request.Request("https://query1.finance.yahoo.com/v1/test/getcrumb", headers=hdr)
        crumb = opener.open(req, timeout=10).read().decode("utf-8").strip()
...
_FUND_CACHE = {}   # sym -> (timestamp, dict)

def fetch_fundamentals(yahoo_sym):
    """Return a dict of fundamentals for a Yahoo symbol (e.g. RELIANCE.NS).
    Free, INR, no API key. Cached 15 min. Returns {} if Yahoo blocks us."""
    now = time.time()
    cached = _FUND_CACHE.get(yahoo_sym)
    if cached and now - cached[0] < 900:
        return cached[1]
    opener, crumb = _yf_session()
    if not opener or not crumb:
        return {}
    try:
        mods = "financialData,defaultKeyStatistics,summaryDetail,price,assetProfile"
        url = (f"https://query2.finance.yahoo.com/v10/finance/quoteSummary/{yahoo_sym}"
               f"?modules={mods}&crumb={urllib.parse.quote(crumb)}")
        hdr = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"}
        with opener.open(urllib.request.Request(url, headers=hdr), timeout=15) as r:
        ...
        def raw(d, k):
            ...
            return v
        out = {
            "longName": pr.get("longName") or pr.get("shortName"),
            "sector": ap.get("sector"),
            "marketCap": raw(pr, "marketCap") or raw(sd, "marketCap"),
            "trailingPE": raw(sd, "trailingPE") or raw(ks, "trailingPE"),
...
def fetch_stock(symbol):
    ...
    if MARKET == "US":
        ...
        if hist is None or hist.empty:
            ...
            if hist is None or hist.empty:
                raise ValueError(f"No data found for '{symbol}'. Please check the symbol (e.g. RELIANCE, TCS).")
    try: info = ticker.info
    except Exception: info = {}
    if not isinstance(info, dict): info = {}
    # Yahoo often returns an EMPTY .info for Indian (.NS/.BO) tickers (rate-limited or blocked).
    # Primary fix: hit Yahoo quoteSummary directly (cookie+crumb) - free, INR, no API key.
    need = ("marketCap","trailingPE","priceToBook","returnOnEquity","debtToEquity","dividendYield")
    if not info or all(info.get(k) is None for k in need):
        ysym = symbol if MARKET=="US" else symbol+".NS"
        fund = fetch_fundamentals(ysym)
        if not fund and MARKET!="US":
            fund = fetch_fundamentals(symbol+".BO")
        for k,v in fund.items():
            if info.get(k) is None:
                info[k]=v
        # last resort: fast_info for market cap / PE
        if all(info.get(k) is None for k in need):
            try:
                fi = dict(ticker.fast_info or {})
                if fi.get("market_cap") is not None: info["marketCap"]=fi["market_cap"]
                if fi.get("trailing_pe") is not None: info["trailingPE"]=fi["trailing_pe"]
            except Exception:
                pass
...
def analyze_mf(scheme_code):
    ...
    try:
        ...
        if not navs or len(navs)<30:
            raise RuntimeError("not enough NAV history from the data source")
...
def analyze(symbol, hist, info):
    ...
    return {"symbol":symbol,"ltp":ltp,"change_pct":change_pct,"dma20":dma20,"dma50":dma50,
        ...
        "rsi":rsi,"atr":atr,"vol_ratio":vol_ratio,"avg_vol":avg_vol,"support":support,"resistance":resistance,
