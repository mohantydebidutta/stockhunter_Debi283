...
SECTOR_INDICES = {
    "NIFTY":"^NSEI","BANK":"^NSEBANK","IT":"^CNXIT","AUTO":"^CNXAUTO",
    "PHARMA":"^CNXPHARMA","FMCG":"^CNXFMCG","METAL":"^CNXMETAL","ENERGY":"^CNXENERGY",
    "REALTY":"^CNXREALTY","FIN":"^CNXFIN","PSUBANK":"^CNXPSUBANK","MEDIA":"^CNXMEDIA",
}
...
def get_sector_indices():
    ...
    for name,sym in SECTOR_INDICES.items():
        try:
            ...
            out.append((name,(ltp-prev)/prev*100))
        except Exception:
            continue
    return out


def show_full_analysis(sym):
    """Poora analysis dikhata hai - Analyze tab aur scan dropdowns dono yahi use karte hain."""
    try:
...
sectors_data=get_sector_indices()
if sectors_data:
    parts=[]
    for name,chg in sectors_data:
        color="🟢" if chg>=0 else "🔴"
        parts.append(f"{color} **{name}** {chg:+.1f}%")
    st.markdown("  •  ".join(parts))

st.divider()

tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs(
    ["🔍 Analyze", "⭐ Perfect Buy", "📊 Market Scan", "💎 Buy the Dip", "💰 Rs 50-300", "⚖️ Compare"])
    ...
    if st.button("Find Perfect Buys", type="primary"):
        if not NIFTY1000:
            ...
            for i,sym in enumerate(syms,1):
                ...
                prog.progress(i/len(syms), f"Scanning {i}/{len(syms)}...")
            prog.empty()
            st.session_state["perfect_results"]=perfects

    # show saved results + dropdown (stays after dropdown change)
    if "perfect_results" in st.session_state:
        perfects=st.session_state["perfect_results"]
        if perfects:
            st.success(f"Found {len(perfects)} perfect buys!")
            st.dataframe(pd.DataFrame(perfects),hide_index=True,use_container_width=True)
            names=[p["Stock"] for p in perfects]
            pick=st.selectbox("👇 Tap a stock to see full analysis", ["-- select --"]+names, key="pick_perfect")
            if pick and pick!="-- select --":
                st.divider()
                show_full_analysis(pick)
        else:
    ...
    if st.button("Run Market Scan", type="primary"):
        if not NIFTY1000:
            ...
            rows=[{"Stock":r["symbol"],"Price":f"Rs {r['ltp']:,.1f}","Day%":f"{r['change_pct']:+.1f}%",
                  ...
                  for r in results[:30]]
            st.session_state["market_results"]=rows

    if "market_results" in st.session_state:
        rows=st.session_state["market_results"]
        if rows:
            st.dataframe(pd.DataFrame(rows),hide_index=True,use_container_width=True)
            names=[r["Stock"] for r in rows]
            pick=st.selectbox("👇 Tap a stock to see full analysis", ["-- select --"]+names, key="pick_market")
            if pick and pick!="-- select --":
                st.divider()
                show_full_analysis(pick)
    ...
    if st.button("Find Dips", type="primary"):
        if not NIFTY1000:
            ...
            for i,sym in enumerate(syms,1):
                ...
                prog.progress(i/len(syms), f"Scanning {i}/{len(syms)}...")
            prog.empty()
            st.session_state["dip_results"]=dips

    if "dip_results" in st.session_state:
        dips=st.session_state["dip_results"]
        if dips:
            st.success(f"Found {len(dips)} dip setups!")
            st.dataframe(pd.DataFrame(dips),hide_index=True,use_container_width=True)
            names=[x["Stock"] for x in dips]
            pick=st.selectbox("👇 Tap a stock to see full analysis", ["-- select --"]+names, key="pick_dip")
            if pick and pick!="-- select --":
                st.divider()
                show_full_analysis(pick)
        else:
...
# ---- Rs 50-300 tab ----
with tab5:
    st.write("Quality stocks priced between **Rs 50 and Rs 300** (no penny stocks, not expensive).")
    n_scan4 = st.slider("How many to scan", 50, len(NIFTY1000) if NIFTY1000 else 300, 300, 50, key="range")
    if st.button("Scan Rs 50-300", type="primary"):
        if not NIFTY1000:
            st.error("Stock list not found.")
        else:
            prog=st.progress(0,"Scanning...")
            results=[]
            syms=NIFTY1000[:n_scan4]
            for i,sym in enumerate(syms,1):
                r=quick_analyze(sym)
                if r and 50 <= r["ltp"] <= 300:
                    results.append(r)
                prog.progress(i/len(syms), f"Scanning {i}/{len(syms)}...")
            prog.empty()
            results.sort(key=lambda x:x["confidence"],reverse=True)
            rows=[{"Stock":r["symbol"],"Price":f"Rs {r['ltp']:,.1f}","Day%":f"{r['change_pct']:+.1f}%",
                   "RSI":f"{r['rsi']:.0f}","Conf%":f"{r['confidence']:.0f}%","Signal":r["verdict"]}
                  for r in results[:30]]
            st.session_state["range_results"]=rows

    if "range_results" in st.session_state:
        rows=st.session_state["range_results"]
        if rows:
            st.success(f"Found {len(rows)} stocks in Rs 50-300 range!")
            st.dataframe(pd.DataFrame(rows),hide_index=True,use_container_width=True)
            names=[r["Stock"] for r in rows]
            pick=st.selectbox("👇 Tap a stock to see full analysis", ["-- select --"]+names, key="pick_range")
            if pick and pick!="-- select --":
                st.divider()
                show_full_analysis(pick)
        else:
            st.warning("No stocks found in Rs 50-300 range right now.")

# ---- Compare tab ----
with tab6:
    st.write("Compare 2-3 stocks side by side.")
    c1,c2,c3 = st.columns(3)
    s1 = c1.text_input("Stock 1", value="TCS", key="cmp1").strip().upper()
    s2 = c2.text_input("Stock 2", value="INFY", key="cmp2").strip().upper()
    s3 = c3.text_input("Stock 3 (optional)", value="", key="cmp3").strip().upper()
    if st.button("Compare", type="primary"):
        syms=[s for s in [s1,s2,s3] if s]
        if len(syms)<2:
            ...
            with st.spinner("Comparing..."):
                for sym in syms:
                    try:
                        s,hist,info=fetch_stock(sym)
                        data.append(analyze(s,hist,info))
                    except Exception:
                        pass
            if len(data)<2:
                st.error("Could not load enough stocks. Check symbols.")
            else:
                best_conf=max(d["confidence"] for d in data)
                table={}
                metrics=[
                    ("Price",lambda d:f"Rs {d['ltp']:,.1f}"),
                    ("Day %",lambda d:f"{d['change_pct']:+.1f}%"),
                    ("Confidence",lambda d:f"{d['confidence']:.0f}%"+(" ⭐" if d['confidence']==best_conf else "")),
                    ("Verdict",lambda d:d['verdict']),
                    ("RSI",lambda d:f"{d['rsi']:.0f}"),
                    ("Weekly trend",lambda d:"UP" if d['weekly_up'] else ("DOWN" if d['weekly_up'] is False else "N/A")),
                    ("vs 20 DMA",lambda d:f"{d['ext_from_dma20']:+.1f}%"),
                    ("From 52W High",lambda d:f"{d['from_high']:.0f}%"),
                    ("Stop-Loss",lambda d:f"Rs {d['stop_loss']:,.1f}"),
                    ("Target 1",lambda d:f"Rs {d['target1']:,.1f}"),
                    ("P/E",lambda d:str(d['pe'])),
                    ("ROE %",lambda d:str(d['roe'])),
                    ("Entry timing",lambda d:d['timing']),
                ]
                for name,fn in metrics:
                    table[name]={d["symbol"]:fn(d) for d in data}
                df=pd.DataFrame(table).T
                st.dataframe(df,use_container_width=True)
                winner=max(data,key=lambda d:d["confidence"])
                st.info(f"Highest confidence: **{winner['symbol']}** ({winner['confidence']:.0f}%). But also check entry timing - the best stock may be over-extended.")
