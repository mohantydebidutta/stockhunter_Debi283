...
def show_multibagger_result():
    ...
    if r["mode"]=="scan":
        st.caption("Tip: tap a stock button below the table to open its full analysis.")
...
def render_stock_market():
    ...
    TAB_NOTES = {
        T_ANALYZE: "📈 Pick any stock to see its full technical + AI analysis, chart and trade plan.",
        T_PERFECT: "⭐ Only stocks that are perfect to buy right now - strong, trending, in entry zone. Liquid names only.",
        T_SCAN:    "🔎 Scan the market and rank every stock by strength. Tap a stock to open its analysis.",
        T_DIP:     "💎 Quality stocks in an uptrend that have cooled off near the 20 DMA - good pullback entries.",
        T_MULTI:   "🚀 AI judges long-term multibagger potential - market size, growth, tailwinds, management, risks.",
        T_SECTOR:  "🔄 See where the money is flowing - sectors ranked by average strength.",
        T_COMPARE: "⚖️ Compare 2-5 stocks side by side, then let AI pick the better long-term bet.",
        T_WATCH:   "⭐ Your saved stocks with live price, signal and one-tap analysis.",
        T_MACRO:   "🌐 Big-picture macro read - rates, inflation, GDP, currency, geopolitics - plus market news.",
        T_PF:      "💼 Add your holdings to get an AI portfolio manager review and a full P&L table.",
    }
    ...
    if cur==T_MULTI:
        ...
        with mc2:
            st.write(""); st.write("")
        depth = st.radio("Scan depth",
            ["Fast (Top 200)", "Deep (Top 500)", "Entire NSE (every listed stock)"],
            horizontal=True, key="mb_depth")
        if depth.startswith("Entire"):
            st.warning("Entire NSE downloads the full list (~2000 stocks) and analyses every one - "
                       "liquid AND illiquid. This can take 15-30 minutes and Yahoo may rate-limit "
                       "partway (it will skip those and keep going). Best run when you have time.")
        else:
            st.caption("Fast/Deep scan the top liquid names quickly. Entire NSE covers everything.")
        b1,b2 = st.columns(2)
        with b1:
        ...
        if mb_scan:
            if not ai_ready(): st.warning("The owner needs to set the Gemini key first.")
                ...
                cnt = 200 if depth.startswith("Fast") else (500 if depth.startswith("Deep") else None)
                run_multibagger_scan(cnt)
...
def clickable_table(df, key, sym_col="Stock"):
    """Show a plain table (no checkbox column) plus a tidy row of Open buttons,
    one per stock, that jump straight to the Analyze tab."""
    st.dataframe(df, use_container_width=True, hide_index=True)
    syms=list(df[sym_col].astype(str))
    # buttons in rows of 4 so they stay neat
    per_row=4
    for start in range(0, len(syms), per_row):
        chunk=syms[start:start+per_row]
        cols=st.columns(per_row)
        for j,s in enumerate(chunk):
            if cols[j].button(f"📈 {s}", key=f"{key}_open_{s}", use_container_width=True):
...
def show_scan_result(kind):
    ...
    if not res: return
    st.caption("Tip: tap a stock button below the table to open it in Analyze.")
...
def render_mf_detail(mf):
    ...
    if HAS_PLOTLY and mf.get("navs"):
        try:
            ...
            st.caption("NAV chart could not be drawn for this fund.")
    elif not HAS_PLOTLY:
